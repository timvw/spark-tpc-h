...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val source = "/Users/timvw/datasets/tpc-h/10";
    val destination = "/Users/timvw/datasets/tpc-h-parquet/10";

    val tpchSchemaProvider = new TpchSchemaProvider(spark.sparkContext, source);
    tpchSchemaProvider.dfMap.foreach(entry => {
      entry._2.repartition(1).write.format("parquet").save(s"${destination}/${entry._1}")
    })
  }
}
